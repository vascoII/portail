...
export interface UseExternalApiQueryOptions {
  enabled?: boolean;
  onSuccess?: (data: any) => void;
  onError?: (error: string) => void;
}

export interface UseExternalApiQueryReturn<T> {
  data: T | null;
  loading: boolean;
  error: string | null;
  lastUpdated?: string;
...
/**
 * Hook générique pour les requêtes GET externes (sans JWT)
 ...
 */
export function useExternalApiQuery<T>(
  queryFn: () => Promise<ApiResponse<T>>,
  options: UseExternalApiQueryOptions = {}
): UseExternalApiQueryReturn<T> {
