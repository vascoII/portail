...
interface AlarmeGaugeProps {
  data: GetParcResponseDto;
  maxValue?: number;
  href?: string;
}

const AlarmeGauge: React.FC<AlarmeGaugeProps> = ({
  data,
  maxValue = 20,
  href = "/immeubles?dysfonctionnements=1",
}) => {
  const nbDysfonctionnements = data.nbDysfonctionnements ?? 0;
  const displayValue = nbDysfonctionnements === -1 ? 0 : nbDysfonctionnements;

  // Couleur du gauge basée sur le nombre de dysfonctionnements
  const getGaugeColor = () => {
    if (displayValue === 0) return "#8e98a2"; // Gris si aucun dysfonctionnement
    if (displayValue <= maxValue * 0.3) return "#10b981"; // Vert si peu de dysfonctionnements
